...
from gaussian_N_sampler import sample_gaussian_ply
import argparse
...
class GUI:
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if target_mask.sum() == 0:
                print('No maskes in frame (skipped)')
                exit()
    ...
    def extract_frame(self, image_path: str) -> int:
        """
        Extracts the 5-digit frame number from the image path and returns it as an integer.
        e.g., '/.../00118.png' -> 118
        """
        filename = os.path.basename(image_path)
        stem, _ = os.path.splitext(filename)
        # If the filename is always a 5-digit number, this simple conversion suffices
        self.time_idx = int(stem)
    ...
    def save_canonical_gaussians(self):
        save_dir = self.opt.outdir


        gaussians_params = self.renderer.gaussians.capture()
        (active_sh_degree,
        xyz,
        features_dc,
        features_rest,
        ...
        spatial_lr_scale) = gaussians_params
        ...
        scales   = self.renderer.gaussians.scaling_activation(scaling)
        rots = self.renderer.gaussians.rotation_activation(rotation)
        opac  = self.renderer.gaussians.opacity_activation(opacity)
        ...
        output_dict = {
            ...
            'opacity': opac.detach().cpu().numpy(),
            'rgb': rgb_dc.detach().cpu().numpy(),
        }

        os.makedirs(save_dir, exist_ok=True)
        subdir = os.path.join(save_dir, 'canonical')
        os.makedirs(subdir, exist_ok=True)

        t = self.time_idx

        output_file = os.path.join(subdir, f'canonical_{t}.pkl')
        
        output_dict = self.sample_gaussians_dict(output_dict, n_samples= opt.num_pts, method="random")
        
        
        with open(output_file, 'wb') as f:
            pickle.dump(output_dict, f)

        print(f"[t={t}] Saved Gaussians at {output_file}")


    def sample_gaussians_dict(self,
        output_dict: dict,
        n_samples: int = 5000,
        method: str = "random",
        opacity_key: str = "opacity"
    ) -> dict:
        xyz = output_dict['mean_3d']
        N = xyz.shape[0]
        if N <= n_samples:
        ...
        if method == "random":
            ...
            weights = np.clip(output_dict[opacity_key].astype(np.float64), 1e-8, None)
            weights /= weights.sum()
            ...
            raise ValueError(f"Unknowen method: {method}")

        return {k: v[idx] for k, v in output_dict.items()}
    ...
    def save_model(self, mode='geo', texture_size=1024):
        ...
        if mode == 'geo':
            ...
            # downscaling (gpu cost)

            # sample_gaussian_ply(path,path,n_samples= opt.num_pts)
            # print(self.renderer.gaussians.get_xyz.shape)
